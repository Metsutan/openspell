...
import { banUserByUsername } from "../../db";
 ...
 *
 * Note: This command only applies account bans (users table), not IP bans.
 */
export const banCommand: CommandHandler = (ctx: CommandContext, args: string[]) => {
  if (args.length < 1) {
    ctx.reply("Usage: /ban <username> [duration]", MessageStyle.Warning);
  ...
  const reason = `Banned by ${ctx.username}`;
  void (async () => {
    const result = await banUserByUsername(targetUsername, parsedDuration, reason);
    if (!result) {
      ctx.reply(`Player "${targetUsername}" was not found`, MessageStyle.Warning);
      return;
    }

    const disconnected = ctx.disconnectPlayer(
      result.userId,
      "Your account has been banned."
    );
    if (disconnected) {
      ctx.reply(`${result.username} was disconnected.`, MessageStyle.Orange);
    }

    if (parsedDuration === null) {
      ctx.reply(`Banned ${result.username} permanently.`, MessageStyle.Red);
      return;
    }

    ctx.reply(`Banned ${result.username} for ${formatDurationShort(maybeDuration)}.`, MessageStyle.Red);
  })().catch((err) => {
    ctx.reply("Failed to ban player due to an internal error.", MessageStyle.Red);
    console.warn("[commands/ban] Failed to ban player:", (err as Error)?.message ?? err);
  });
};
